...
pub extern "C" fn codexbar_portable_core_execute(request_json: *const c_char) -> *mut c_char {
    ...
    let response_json = serde_json::to_string(&response).unwrap_or_else(|error| {
        format!(
            "{{\"ok\":false,\"error\":{{\"code\":\"serializationFailure\",\"message\":\"{error}\"}}}}"
        )
    });
...
/// # Safety
///
/// `value` must be a pointer previously returned by
/// `codexbar_portable_core_execute` via `CString::into_raw`, and it must be
/// freed exactly once.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn codexbar_portable_core_free_string(value: *mut c_char) {
    if value.is_null() {
        return;
    }
    unsafe {
