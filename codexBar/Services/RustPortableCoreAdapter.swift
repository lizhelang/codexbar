...
final class RustPortableCoreAdapter {
    ...
    func parseOAuthInteropBundle(
        _ request: PortableCoreOAuthInteropBundleParseRequest,
        buildIfNeeded: Bool = false
    ) throws -> PortableCoreOAuthInteropBundleParseResult {
    ...
    func planOpenRouterModelSelection(
        _ request: PortableCoreOpenRouterModelSelectionPlanRequest,
        buildIfNeeded: Bool = false
    ) throws -> PortableCoreOpenRouterModelSelectionPlanResult {
        try self.call(operation: .planOpenRouterModelSelection, payload: request, buildIfNeeded: buildIfNeeded)
    }
    ...
    func resolveLocalCostCachePolicy(
        _ request: PortableCoreLocalCostCachePolicyRequest,
        buildIfNeeded: Bool = false
    ) throws -> PortableCoreLocalCostCachePolicyResult {
        try self.call(operation: .resolveLocalCostCachePolicy, payload: request, buildIfNeeded: buildIfNeeded)
    }

    func mergeHistoricalModels(
        _ request: PortableCoreHistoricalModelsMergeRequest,
        buildIfNeeded: Bool = false
    ) throws -> PortableCoreHistoricalModelsMergeResult {
    ...
    func resolveGatewayStickyKey(
        _ request: PortableCoreGatewayStickyKeyResolutionRequest,
        buildIfNeeded: Bool = false
    ) throws -> PortableCoreGatewayStickyKeyResolutionResult {
    ...
    func renderGatewayWebSocketHandshake(
        _ request: PortableCoreGatewayWebSocketHandshakeRequest,
        buildIfNeeded: Bool = false
    ) throws -> PortableCoreGatewayWebSocketHandshakeResult {
    ...
    func parseGatewayWebSocketFrame(
        _ request: PortableCoreGatewayWebSocketFrameParseRequest,
        buildIfNeeded: Bool = false
    ) throws -> PortableCoreGatewayWebSocketFrameParseResult {
    ...
    func parseOpenRouterModelCatalog(
        _ request: PortableCoreOpenRouterModelCatalogParseRequest,
        buildIfNeeded: Bool = false
    ) throws -> PortableCoreOpenRouterModelCatalogParseResult {
        try self.call(operation: .parseOpenRouterModelCatalog, payload: request, buildIfNeeded: buildIfNeeded)
    }
    ...
    func buildOAuthAccountFromTokens(
        _ request: PortableCoreOAuthAccountBuildRequest,
        buildIfNeeded: Bool = false
    ...
    func inspectOAuthTokenMetadata(
        ...
        buildIfNeeded: Bool = false
    ) throws -> PortableCoreOAuthTokenMetadataResult {
        try self.call(operation: .inspectOAuthTokenMetadata, payload: request, buildIfNeeded: buildIfNeeded)
    }
